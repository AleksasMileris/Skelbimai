...
class CategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('Categories/index', [
            'categories'=>Category::filter($filter)->order($order)->with('posts')->get(),
            'fil'=>$filter,
            'ord'=>$order
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $old=Category::all()->where("name","like",$request->name);
        if( count($old) > 0 ){
            return to_route('categories.error');
        }else{
    ...
    public function show(Category $category,Request $request)
    {
        ...
        $filter=$request->session()->get("posts_filter",$emFilter);

        return Inertia::render('Categories/Show', [

            'category'=>Category::with('posts')->where("id","like",$category->id)->first(),
            'posty'=>$filter->name!=""?$category->posts()->where('name', 'like', "%$filter->name%")->get():null,
            'fil'=>$filter
        ]);
    }
    ...
    public function filterPosts(Request $request){
        $filter=new \stdClass();
        $filter->name=$request->name;
        $request->session()->put("posts_filter",$filter);
